resshape_data <- function(file){
  ...
  try({res$Bicoid = with(res,1*pbcd+1*pbcd1+2*pbcd2+3*pbcd3+4*pbcd4+5*pbcd5+6*pbcd6+6*(pbcd6phb1+pbcd6phb2))/2.5})
  try({res$Hunchback = with(res,1*phb+1*pbcd6phb1+2*pbcd6phb2)/2.5})
  try({res$Caudal = with(res,pcad)/2.5})
  try({res$Nanos = with(res,1*pnos+1*boundmhb+1*boundmbcd)/2.5})
