resshape_data <- function(file){
  ...
  res[2] = as.numeric(levels(res$distance))[res$distance]
    with(res,res$Bicoid = 1*pbcd+1*pbcd1+2*pbcd2+3*pbcd3+4*pbcd4+5*pbcd5+6*pbcd6+6*(pbcd6phb1+pbcd6phb2))
    with(res,res$Hunchback = 1*pbcd6phb1+2*pbcd6phb2)
    with(res,res$Caudal = pcad)
    with(res,res$Nanos = 1*pnos+1*boundmhb+1*boundmbcd)
  return(res)
}